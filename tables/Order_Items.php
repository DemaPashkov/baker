...
<div class="blocksss">
    ...
    <div class="blocksss">
        <div>
            <div class="naz_tabl"><h2>Order_Items</h2></div>
            <div>
                <table>
                    ...
                    <?php 
                        ...
                        $suppliers = mysqli_query($conn, "SELECT * FROM Order_Items" );
                        for($data = []; $row = mysqli_fetch_assoc($result); $data [] = $row);
                        foreach($data as $supplier) {
                            echo "<tr class='read_tabl'>";
                            echo "<td>" .  $supplier['order_id'] . "</td>";
                            echo "<td>" .  $supplier['product_id'] . "</td>";
                            echo "<td>" .  $supplier['item_quantity'] . "</td>";
...
if (!empty($_POST['submit'])){
    if ((!empty($_POST['product_id']) and !empty($_POST['item_quantity']))){
        $suppliers=$_POST['product_id'];
        $item_quantity=$_POST['item_quantity'];
       
        mysqli_query($conn, "INSERT INTO `Order_Items` (`order_id`, `product_id`, `item_quantity`) VALUES (NULL, '$suppliers', '$item_quantity')");
        header("Refresh:0");
    }else {
...
?>
    <div class="dobaxit_danne">
        ...
        <form action="#" method="post">
            <p>product_id</p>
            <input class="form_for-text" type="text" name="product_id"><br>
            <p>item_quantity</p>
            <input class="form_for-text" type="text" name="item_quantity"><br><br>
            <input class="save_main-submit" type="submit" name="submit" value="Добавить">
        </form>
    </div>
            <?php
            if(!empty($_GET['red_id'])){
                $query = "SELECT * FROM Order_Items where order_id={$_GET['red_id']}";
                $result = mysqli_query($conn, $query);
                $suppliers = mysqli_query($conn, "SELECT * FROM Order_Items" );
                for($data = []; $row = mysqli_fetch_assoc($result); $data [] = $row);
                echo "<form method='POST'>";
                    foreach($data as $supplier) {
                        ...
                        <div class='dobaxit_danne'> 
                            ...
                            <p>product_id</p>
                            <input class='form_for-text' type='text' required name='product_id' value='{$supplier['product_id']}'/>
                            <p>item_quantity</p>
                            <input class='form_for-text' type='text' required name='item_quantity' value='{$supplier['item_quantity']}'/><br>
                        ";
                    }
                ...
                if (!empty($_POST['update'])){
                    $suppliers=$_POST['product_id'];
                    $item_quantity=$_POST['item_quantity'];
                    mysqli_query($conn, "UPDATE `Order_Items` SET `product_id` = '$suppliers',`item_quantity` = '$item_quantity'  where order_id = {$_GET['red_id']}");
                    header("Refresh:0");
                }
            }
            ?>
            <?php
                ...
                $supplier = mysqli_query($conn, "DELETE FROM Order_Items WHERE order_id = {$_GET['del_id']}");        
                }   
            ?>
            </div>
        </div>
